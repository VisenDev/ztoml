...
pub fn parseToml(comptime T: type, a: std.mem.Allocator, input_string: []const u8) !std.json.Parsed(T) {
    const raw_string = tomlToJson(input_string.ptr);
    if (raw_string == null) {
...
pub extern fn tomlToJson(input: [*c]const u8) [*c]u8;
...
pub const __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ = @import("std").zig.c_translation.promoteIntLiteral(c_int, 130200, .decimal);
